...
public class TokenRenewalTest_existingUgi_oldApi {

   public static void main(String[] args) throws Exception {
     int failFlags = 10000; // test result init but not updated yet
     JobConf conf = new JobConf(TokenRenewalTest_existingUgi_oldApi.class);
     ...
     FileInputFormat.setInputPaths(conf, new Path("/data/in"));
    ...
    if (renewTimeRm > 1357252344100L) 
    {
     ...
     conf.setJobName("TokenRenewalTest_existingUgi_oldApi_job2");
     if ( outpath.getFileSystem(conf).isDirectory(outpath) ) {
       outpath.getFileSystem(conf).delete(outpath, true);
       System.out.println("Info: deleted output path: " + outpath );
     }
