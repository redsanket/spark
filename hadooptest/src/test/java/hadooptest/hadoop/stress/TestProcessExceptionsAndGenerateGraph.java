...
public class TestProcessExceptionsAndGenerateGraph extends TestSession {
	...
	private String table1OutputGraphFileName = TestSession.conf.getProperty("WORKSPACE") + "/target/surefire-reports/table1.png";
	private String table2OutputGraphFileName = TestSession.conf.getProperty("WORKSPACE") + "/target/surefire-reports/table2.png";
	private String perlScriptForGraphGeneration = TestSession.conf.getProperty("WORKSPACE") + "/scripts/generateGraphForMonitoring.pl";
			...
			for (String clazz : classes) {
				...
				for (String test : tests) {
					...
					for (String testRun : testRuns) {
						TestSession.logger.trace("Processing testRun:" + testRun);
						...
						while ((line = bufferedReader.readLine()) != null) {
							 ...
							 * new exceptions X-Axis: Exceptions, Y-Axis:
							 * exception count
							 */
							...
							if (table2TestRunHash.containsKey(testRun)) {
								...
								// Make an entry is exception not present
								if (!exceptionPresent) {
						...
						/*
						 * Care for Table-1 here (X:axis T1, T2, T3..) Y:Axis:
						 * Count of Exceptions encountered per test
						 */
	...
	public void generateGraphs() throws Exception{
		String outputBaseDirPath = TestSession.conf.getProperty("WORKSPACE") + "/target/surefire-reports";
		...
		runPerlScriptToGenerateGraph(table1GraphTitle, table1XAxisTitle,
				table1YAxisTitle, table1InputDataForGraphGeneration,
				table1OutputGraphFileName);
		generateTabe2Data(table2InputDataForGraphGeneration);
		runPerlScriptToGenerateGraph(table2GraphTitle, table2XAxisTitle,
				table2YAxisTitle, table2InputDataForGraphGeneration,
				table2OutputGraphFileName);

	}
	public void runPerlScriptToGenerateGraph(String graphTitle,
		...
		sb.append(" ");
		sb.append(perlScriptForGraphGeneration);
		sb.append(" ");
		sb.append("-t");
		sb.append(" ");
		...
		Process process = TestSession.exec.runProcBuilderGetProc(sb.toString().split("\\s+"));
		printResponseAndReturnItAsString(process);
		TestSession.logger.trace("Xgit value:" + process.exitValue());
	}
	...
	public void generateTabe1Data(String dataOutputFile)
		...
		for (String aRun : table1TestRunHash.keySet()) {
			...
			for (Object aTestNameOnXAxis : testFootPrintArray) {
				for (HashMap<String, Integer> dataNodeInTable1 : table1TestRunHash
						.get(aRun)) {
					if (dataNodeInTable1.containsKey((String) aTestNameOnXAxis)) {
						nodeFoundInRun = true;
						printWriter.print(dataNodeInTable1
