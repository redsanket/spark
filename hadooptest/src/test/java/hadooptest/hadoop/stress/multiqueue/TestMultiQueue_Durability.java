...
//				"/conf/MultiQueueDurabilityConf/multiqueue-durability-capacity-scheduler.xml";
////		String sourceFile = TestSession.conf.getProperty("WORKSPACE") +
...
//		Thread.sleep(30000);
	...
	private void startJobs(WordCountJob jobUserDefault, int i){
		try{
			...
			input_index = random.nextInt(TotalFileNum-5)+5;
	...
	private void assertJobs(WordCountJob jobUserDefault, int i, int j){	
		try{
			assertTrue("WordCount job (default user) was not assigned an ID within 120 seconds." + "i = " + i +", j = " + j, 
			...
			System.out.println("################################################Job(" +i+","+j+")"+"successed!");
		} catch (Exception e){
			TestSession.logger.error("Exception failure.", e);
			fail();
		}
