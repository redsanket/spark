...
public class FsckResponseBO {
	...
	public class FsckFileDetailsBO {
		 ...
		 * /HTF/testdata/dfs/_big_file_10dot7GB 11811160064 bytes, 88 block(s):  OK
		 * /HTF/testdata/dfs/_big_file_11GB 11811160064 bytes, 88 block(s):  OK
		 * /HTF/testdata/dfs/_file_128MB 1073741824 bytes, 8 block(s):  OK

		 */
		public FsckFileDetailsBO(String aLineFromFsckResponse){
			this.fileName = aLineFromFsckResponse.replaceAll(pattern, "$1");
			this.sizeInBytes = Double.parseDouble(aLineFromFsckResponse.replaceAll(pattern, "$2"));
			this.numBlocks = Integer.parseInt(aLineFromFsckResponse.replaceAll(pattern, "$3"));
...
//		
//		private static final String blocksAndRacksPattern = "([\\d]+)\\.\\s+([\\w\\-\\.]+):([\\w\\_]+)\\s+len=([\\d]+)\\s+repl=([\\d]+)\\s+\\[\\/([\\d\\.]+)\\/([\\d\\.\\:]+),\\s+\\/([\\d\\.]+)\\/([\\d\\.\\:]+),\\s+\\/([\\d\\.]+)\\/([\\d\\.\\:]+)\\]";
		 ...
		 * 
		 */
		FsckBlockDetailsBO(String aLineFromFsckResponse,
				boolean racksArgPassed) {
			if (racksArgPassed) {
				...
				this.blockPool = aLineFromFsckResponse.replaceAll(blocksAndRacksPattern,
						"$2");
				this.blockName = aLineFromFsckResponse.replaceAll(blocksAndRacksPattern,
						"$3");				
				this.length = Integer.parseInt(aLineFromFsckResponse.replaceAll(
						blocksAndRacksPattern, "$4"));
	...
	public class FsckSummaryBO {
		...
		void setAppropriateMember(String aLineFromFsckResponse) {
			...
			if (aLineFromFsckResponse.matches(pattern)) {
				this.totalSizeInBytes = Double.parseDouble(aLineFromFsckResponse
						.replaceAll(pattern, "$1"));
				return;
			}
			pattern = "\\s+Total dirs:\\s+(\\d+)";
			if (aLineFromFsckResponse.matches(pattern)) {
				this.totalDirs = Integer.parseInt(aLineFromFsckResponse.replaceAll(
						pattern, "$1"));
				return;
			}
			...
			if (aLineFromFsckResponse.matches(pattern)) {
				this.totalBlocksValidated = Integer.parseInt(aLineFromFsckResponse
						.replaceAll(pattern, "$1"));
			...
			if (aLineFromFsckResponse.matches(pattern)) {
				this.minimallyReplicatedBlocks = Integer
						.parseInt(aLineFromFsckResponse.replaceAll(pattern, "$1"));
				return;
			}

			pattern = "\\s+Over-replicated blocks:\\s+(\\d+)\\s+\\(\\d+\\.\\d\\s+%\\)";
			if (aLineFromFsckResponse.matches(pattern)) {
				this.overReplicatedBlocks = Integer.parseInt(aLineFromFsckResponse
						.replaceAll(pattern, "$1"));
				return;
			}
			...
			if (aLineFromFsckResponse.matches(pattern)) {
				this.underReplicatedBlocks = Integer.parseInt(aLineFromFsckResponse
						.replaceAll(pattern, "$1"));
				return;
			}
			...
			if (aLineFromFsckResponse.matches(pattern)) {
				this.misReplicatedBlocks = Integer.parseInt(aLineFromFsckResponse
						.replaceAll(pattern, "$1"));
				return;
			}
			...
			if (aLineFromFsckResponse.matches(pattern)) {
				this.averageBlockReplication = Float
						.parseFloat(aLineFromFsckResponse.replaceAll(pattern, "$1"));
				return;
			}
