<nav class="navbar navbar-inverse">
  ...
  %>
  <%
  			SELECT_QUERY = "SELECT DISTINCT * from integration_test where hadoopVersion=\"" + buildQueryParameterValue.trim() + "\"  ORDER BY startTime ;";
  		} else {
  %>
  <%
  			SELECT_QUERY = "SELECT DISTINCT * from integration_test ORDER BY startTime";	
  		}
  %>
  <%
  ...
  <%
  				showRecordsPerPageParameterValue = Integer.toString(DEFAULT_ROW_SIZE);
  		}
  %>
		 ...
		 <script type="text/javascript">
		 	$(document).ready(function() {
     			var table = $('#job-completed-table').dataTable({
     				 "iDisplayLength": <%= showRecordsPerPageParameterValue %>
     			});
     			<%
     				if (buildQueryParameterValue != null) {
     			%>
			...
			%>
				...
				<thead>
					<tr>
						...
						<th class="text-center">Core build #</th>
						<th class="text-center">jobStarted</th>
						<th class="text-center">Data Available</th>
				...
				<tbody>
					...
					<%
							...
							String coreBuildNo = resultSet.getString("hadoopVersion");
		...
		%>
					<div class="alert alert-info">
  						<strong>Info!</strong> Current frequency : <%= currentHrFrequency %> job is not yet started, wait...
					</div>
					<img style="float: center; margin: 0px 0px 10px 20px;" src="./images/circle_grey.png" width="100" title="Job has not yet started." />
					<img style="float: center; margin: 0px 0px 10px 20px;" src="./images/circle_grey.png" width="100" title="Job has not yet started." />
					<img style="float: center; margin: 0px 0px 10px 20px;" src="./images/circle_grey.png" width="100" title="Job has not yet started." />
					<img style="float: center; margin: 0px 0px 10px 20px;" src="./images/circle_grey.png" width="100" title="Job has not yet started." />
					<img style="float: center; margin: 0px 0px 10px 20px;" src="./images/circle_grey.png" width="100" title="Job has not yet started." />
