...
abstract class KVDataTest(sc: SparkContext, dataType: String = "string") {
  ...
  val DELETE_DATA = ("delete-data", "Delete the generated data if using 'hdfs' as persistence type")
  ...
  val booleanOptions = Seq(HASH_RECORDS, DELETE_DATA)
  ...
  def initialize(args: Array[String]) = {
    ...
    deleteData = optionSet.has(DELETE_DATA._1)
  }
  ...
  def deleteInputData() = {
    ...
    if (persistenceType == "hdfs" && deleteData) {
      DataGenerator.deleteHdfsDir(storageLocation)
    }
  }
  ...
  def run() = {
    ...
    options.foreach {case (k, v) => println(s"$k -> $v")}

    val results = (1 to numTrials).map { t =>
      ...
      println(s"trial $t spent $time s")
    }
  }
}
