...
"This app helps identify wasted grid resources by showing the difference between the container size used by individual Map/Reduce apps and how big a container was actually needed to run those apps. It also estimates cost savings from "right-sizing" the container size of your apps, based on the Hadoop TCO cost model."
...
The difference between the two counters is how much off-heap memory your task is using.  This needs to be accounted for in the gap between the container and heap settings.  For instance, if you use 200MB of off heap memory, the container should be ``512+200MB`` larger than the heap memory setting. 
...
Throughput, (``1 - GC/totalTime``), represents the time the CPU is doing useful work.  Below ``~90%`` indicates significant time spent doing collections, and you may want to try a larger heap. 
...
Doppler's project grid pages have information about your queue usage and computed total cost of ownership (TCO). If you're under your quota limits, you may not need to optimize your jobs. Not wasting resources is always desirable, though.
