if [ "$CHECKSSHCONNECTIVITY" = true ]
then
    for h1 in $jobtrackernode
    do
	echo == checking that ssh to $h1 works.

	if [ "`echo date +%Y | ssh $jobtrackernode su - $MAPREDUSER `" != "$yr" ]
	then
	...
	do
		echo "==== testing $h1 -> $h2 for user=$MAPREDUSER."
	        if [ "`echo date +%Y | ssh $jobtrackernode su - $MAPREDUSER ssh $h2 `" != "$yr" ]
		then
    ...
    echo == not explicitly checking ssh connectivity.
fi
