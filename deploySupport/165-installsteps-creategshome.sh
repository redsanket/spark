...
fanout_root "if [ ! -d ${GSHOME} ] ; then  if [ $(df -k /home | awk ' /\// { print $2 } ') -le 100000000 ] ; then \
mkdir -p /grid/0/gshome; ln -s /grid/0/gshome ${GSHOME}; else mkdir -p ${GSHOME}; fi; fi"

fanoutGW_root "if [ ! -d ${GSHOME} ] ; then if [ $(df -k /home | awk ' /\// { print $2 } ') -le 100000000 ] ; then \
mkdir -p /grid/0/gshome; ln -s /grid/0/gshome ${GSHOME}; else mkdir -p ${GSHOME}; fi; fi"

fanout_root "if [ ! -d /home/gs/var/jobstatus ] ; then \
