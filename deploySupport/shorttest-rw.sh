...
cd ${yroothome}
if [ -e share/hadoop-current ]; then
...
if [ "$readonly" = true ]; then
    ...
    if [ "$checkstatus" = "Safe mode is OFF" ]; then
...
jar $HADOOP_MAPRED_HOME/share/hadoop/mapreduce/hadoop-mapreduce-examples-*.jar \
randomwriter \
-Dmapreduce.job.queuename=grideng \
-D mapreduce.randomwriter.totalbytes=$((128*1024)) \
$yarnparms \
$fname
        RC=$?
        set +x
        [ $RC -eq 0 ] && export written=`expr $written + 1`
   fi
done
...
if [ "$written" -gt 0  -a "$read" = "$written" ]; then
...
$outdir
    ...
    echo "$HADOOP_COMMON_HOME/bin/hadoop \
jar $HADOOP_MAPRED_HOME/share/hadoop/mapreduce/hadoop-mapreduce-examples-*.jar \
randomwriter \
-Dmapreduce.job.queuename=grideng \
$yarnparms \
-D mapreduce.randomwriter.totalbytes=$((1024*1024)) \
rw1-$tmpfile.1mb"
    $HADOOP_COMMON_HOME/bin/hadoop \
jar $HADOOP_MAPRED_HOME/share/hadoop/mapreduce/hadoop-mapreduce-examples-*.jar \
randomwriter \
