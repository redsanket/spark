...
mkyicf() {
	...
	echo GROUP = root
	echo PERM = 0444
        allfields="CHECKSSHCONNECTIVITY CLUSTER BUILD_NUMBER JOB_NAME \
CONFIGUREJOBTRACKER CONFIGURENAMENODE ENABLE_HA HADOOP_INSTALL_STRING \
HADOOP_CORETREE_INSTALL_STRING HADOOP_MVN_INSTALL_STRING \
HADOOP_CONFIG_INSTALL_STRING  HERRIOT_CONF_ENABLED INSTALLLOCALSAVE \
INSTALLNEWPACKAGES INSTALLYUMMERGE KILLALLPROCESSES REMOVE_EXISTING_LOGS \
REMOVEEXISTINGDATA REMOVERPMPACKAGES RUNKINIT RUNSIMPLETEST STARTYARN \
STARTNAMENODE YINSTSELFUPDATE HDFSUSER  MAPREDUSER PIGVERSION HDFSPROXYVERSION \
ZOOKEEPERVERSION DISTCPVERSION LOG_COLLECTORVERSION HIVEVERSION STARLINGVERSION \
HCATVERSION VAIDYAVERSION HITVERSION INSTALL_HIT_TEST_PACKAGES \
EXCLUDE_HIT_TESTS RUN_HIT_TESTS DATESTRING HCAT_TEST_PKG HIVE_TEST_PKG \
DISTCP_TEST_PKG LOG_COLLECTOR_TEST_PKG VAIDYA_TEST_PKG PIG_TEST_PKG \
OOZIE_TEST_PKG GDM_TEST_PKG NOVA_TEST_PKG HDFSPROXY_TEST_PKG \
HADOOPCORE_TEST_PKG GDMVERSION NOVAVERSION OOZIEVERSION OOZIE_SERVER \
OOZIE_TEST_SUITE HIT_DEPLOYMENT_TAG CREATEGSHOME LOCAL_CONFIG_INSTALL_STRING \
SEND_LOG_TO_STDOUT NO_CERTIFICATION QA_PACKAGES INSTALL_GW_IN_YROOT \
CLEANLOCALCONFIG USE_DEFAULT_QUEUE_CONFIG LOCAL_CONFIG_PKG_NAME GRIDJDK_VERSION \
HBASEVERSION HBASE_TEST_PKG ADMIN_HOST HBASE_SHORTCIRCUIT \
CREATE_NEW_CLUSTER_KEYTAB HIVE_SERVER2_VERSION HIVE_VERSION RHEL6NATIVE \
HCATIGORTAG HIVEIGORTAG OOZIEIGORTAG HADOOPVERSION FULLHADOOPVERSION \
INSTALL_TEZ TEZVERSION HOMEDIR START_STEP"
	for i in $allfields
	do
