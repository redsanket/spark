...
then
    ...
    fanoutnogw "rsync -a $ADMIN_HOST::tmp/deploy.$cluster.remove.old.packages.sh   /tmp/deploy.$cluster.remove.old.packages.sh && GSHOME=$GSHOME yroothome=$yroothome sh /tmp/deploy.$cluster.remove.old.packages.sh && rm /tmp/deploy.$cluster.remove.old.packages.sh "
    ssh $gateway  "rsync -a $ADMIN_HOST::tmp/deploy.$cluster.remove.gateway.old.packages.sh   /tmp/deploy.$cluster.remove.gateway.old.packages.sh && sh /tmp/deploy.$cluster.remove.gateway.old.packages.sh && rm /tmp/deploy.$cluster.remove.gateway.old.packages.sh "
else
