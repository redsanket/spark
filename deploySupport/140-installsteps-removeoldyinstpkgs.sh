...
then
    ...
    ssh $gateway  "scp  $ADMIN_HOST:/grid/0/tmp/:tmp/deploy.$cluster.remove.gateway.old.packages.sh   /tmp/deploy.$cluster.remove.gateway.old.packages.sh && sh /tmp/deploy.$cluster.remove.gateway.old.packages.sh && rm /tmp/deploy.$cluster.remove.gateway.old.packages.sh "
else
