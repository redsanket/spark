...
if [ $SPARKVERSION != none ] && [ $STACK_COMP_INSTALL_SPARK != false ]; then
    ...
    cmd="export SPARK_CONF_DIR=/home/gs/conf/spark/latest ; \
      ...
      --num-executors 2 --executor-memory 2g --queue default --conf spark.executorEnv.JAVA_HOME=$JAVA_HOME \
      --conf spark.yarn.appMasterEnv.JAVA_HOME=$JAVA_HOME $SPARK_HOME/lib/spark-examples-*.jar ; \
