...
fanout "rsync -a $ADMIN_HOST::tmp/scripts.deploy.$cluster/cleangrid.sh   /tmp/cleangrid.sh && export GSHOME=$GSHOME && export HDFSUSER=$HDFSUSER && export MAPREDUSER=$MAPREDUSER &&  sh /tmp/cleangrid.sh && rm /tmp/cleangrid.sh "
