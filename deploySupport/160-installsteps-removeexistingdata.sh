...
if [ "$REMOVEEXISTINGDATA" == "true" ]; then
    ...
    fanoutcmd "scp -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null /grid/0/tmp/scripts.deploy.$cluster/cleangrid.sh __HOSTNAME__:/tmp/cleangrid.sh" "$HOSTLIST"
