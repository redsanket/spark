...
if [ "$REMOVEEXISTINGDATA" == "true" ]; then
    ...
    fanout_root "export GSHOME=$GSHOME && export HDFSUSER=$HDFSUSER && export MAPREDUSER=$MAPREDUSER && \
sh /tmp/cleangrid.sh && rm /tmp/cleangrid.sh"
    ...
    fanoutscp "$scriptdir/cleangrid.sh" "/tmp/cleangrid.sh" "$HOSTLIST"
    fanout_root "export GSHOME=$GSHOME && export HDFSUSER=$HDFSUSER && export MAPREDUSER=$MAPREDUSER && \
sh /tmp/cleangrid.sh yarn && rm /tmp/cleangrid.sh"
else
