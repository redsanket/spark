...
(
    ...
    then
        ...
        if [[ ${DOCKER_IMAGE_TAG_TO_USE} == 'rhel7' || ${DOCKER_IMAGE_TAG_TO_USE} == 'rhel6' ]]; then
            ...
            echo "$yinst set -root ${yroothome} \\"
            echo "    $confpkg.TODO_YARN_NODEMANAGER_RUNTIME_LINUX_DOCKER_IMAGE_NAME=docker-registry.ops.yahoo.com:4443/hadoop/docker_configs/${DOCKER_IMAGE_TAG_TO_USE}:current \\"
            echo "    $confpkg.TODO_YARN_NODEMANAGER_RUNTIME_LINUX_DOCKER_ALLOWED_IMAGES=docker-registry.ops.yahoo.com:4443/hadoop/docker_configs/${DOCKER_IMAGE_TAG_TO_USE}:current \\"
        else
