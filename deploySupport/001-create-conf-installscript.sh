...
(
    ...
    then
        ...
        if [[ ${DOCKER_IMAGE_TAG_TO_USE} == 'rhel7' || ${DOCKER_IMAGE_TAG_TO_USE} == 'rhel6' ]]; then
            ...
            echo "$yinst set -root ${yroothome} \\"
            echo "    $confpkg.TODO_YARN_NODEMANAGER_RUNTIME_LINUX_DOCKER_IMAGE_NAME=hadoop/docker_configs/${DOCKER_IMAGE_TAG_TO_USE}:current \\"
            echo "    $confpkg.TODO_YARN_NODEMANAGER_RUNTIME_LINUX_DOCKER_ALLOWED_IMAGES=hadoop/docker_configs/${DOCKER_IMAGE_TAG_TO_USE}:current \\"
        else
            echo "get specific ${DOCKER_IMAGE_TAG_TO_USE} tag image"
            echo "$yinst set -root ${yroothome} \\"
            echo "    $confpkg.TODO_YARN_NODEMANAGER_RUNTIME_LINUX_DOCKER_IMAGE_NAME=hadoop/docker_configs/${DOCKER_IMAGE_TAG_TO_USE} \\"
            echo "    $confpkg.TODO_YARN_NODEMANAGER_RUNTIME_LINUX_DOCKER_ALLOWED_IMAGES=hadoop/docker_configs/${DOCKER_IMAGE_TAG_TO_USE} \\"
        fi
    fi
