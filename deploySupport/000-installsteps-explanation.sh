...
Keep in mind that the deploy-job actually runs on hadoopre-something.
...
On $ADMIN_HOST, now running as root, it has the access ($ADMIN_HOST has .ssh access
to the entire QA/QE machines as root) and privs (yinst runs as root)
to run installgrid.sh.
