...
then
    ...
    if [ "$SPARK_SHUFFLE_VERSION" != "none" ]; then
        ...
        spark_shuffle_cmd="$yinst install -yes -os rhel-7.x -root ${yroothome} yspark_yarn_shuffle-$SPARK_SHUFFLE_VERSION -br quarantine -same -live -downgrade"
    fi

    cmd="$yinst install -yes -os rhel-7.x -root ${yroothome}  $HADOOP_INSTALL_STRING -same -live -downgrade -br quarantine"
...
#    [ -f ${yroothome}/share/hadoop/share/hadoop/hdfs/lib/$f ] || scp $ADMIN_HOST:/grid/0/tmp/$f  ${yroothome}/share/hadoop/share/hadoop/hdfs/lib/$f  "
   ...
   then
        ...
        slowfanout "$yinst install -yes -os rhel-7.x -root ${yroothome}  $QA_PACKAGES -same -live"
