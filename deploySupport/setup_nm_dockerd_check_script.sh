...
echo "Checking if we need to startup image container daemon on node $HOSTNAME"
...
if [[ "$OS_VER" =~ ^7. ]] && [[ "$ALLOWED_RUNTIMES" =~ "fsimage" ]]; then
  ...
  echo "OS is $OS_VER, ALLOWED_RUNTIMES is not set for containers, not attempting start"
elif [[ "$OS_VER" =~ "6." ]]; then
  echo "OS is $OS_VER, RHEL6 does not support containers, not attempting start"
else
