...
public class ExecuteCommand extends AbstractCommand {
    ...
    private class DefaultStrategy implements Strategy {
        ...
        public void execute(CommandCall commandCall, Evaluator evaluator, ResultRecorder resultRecorder) {
            ...
            for(int i=0; i<size; i++){
            	CommandCall cc=childCommands.get(i);
            	String attr = cc.getElement().getAttributeValue("order");
            	if(attr!=null && attr.equals("first")){
            		cc.execute(evaluator, resultRecorder);
            	}
            }
    ...
    protected void announceExecuteCompleted(Element element) {
