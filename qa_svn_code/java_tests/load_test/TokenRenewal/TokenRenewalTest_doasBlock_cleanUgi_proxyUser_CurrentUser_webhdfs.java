// TokenRenewalTest_doasBlock_cleanUgi_proxyUser_CurrentUser - acquire and attempt renew of HDFS 
// and RM DT tokens from a current user context but using a new remoteUser UGI to clear the sec 
...
public class TokenRenewalTest_doasBlock_cleanUgi_proxyUser_CurrentUser_webhdfs extends Configured implements Tool {

  int failFlags = 10000; // test result init but not updated yet

  public static void main(String[] args) throws Exception {
    int ret = ToolRunner.run(new TokenRenewalTest_doasBlock_cleanUgi_proxyUser_CurrentUser_webhdfs(), args);
  ...
  public class DoasUser {
    ...
    DoasUser () throws Exception {
      ...
      // get a proxy UGI for mapredqa, since no creds are passed in we have to get tokens
      // using the TGT fallback for the login (or Current) user
      try { ugi = UserGroupInformation.createProxyUser("mapredqa@DEV.YGRID.YAHOO.COM", UserGroupInformation.getCurrentUser()); }
      //tryCurrentUser    try { ugi = UserGroupInformation.createProxyUser("mapredqa@DEV.YGRID.YAHOO.COM", UserGroupInformation.getLoginUser()); }
      ...
      String retVal = ugi.doAs(new PrivilegedExceptionAction<String>() {
        public String run() throws Exception {
          ...
          System.out.println("From DoasProxyUser... my Creds say i'm: " + UserGroupInformation.getCurrentUser() + " and I have " + doasCreds.numberOfTokens() + " tokens");
           ...
           jobDoas.setJarByClass(TokenRenewalTest_doasBlock_cleanUgi_proxyUser_CurrentUser_webhdfs.class);
           jobDoas.setJobName("TokenRenewalTest_doasBlock_cleanUgi_proxyUser_CurrentUser_webhdfs_wordcountOrigUser_job1");
           ...
           jobDoas2.setJarByClass(TokenRenewalTest_doasBlock_cleanUgi_proxyUser_CurrentUser_webhdfs.class);
           jobDoas2.setJobName("TokenRenewalTest_doasBlock_cleanUgi_proxyUser_CurrentUser_webhdfs_wordcountDoasUser_job2");
