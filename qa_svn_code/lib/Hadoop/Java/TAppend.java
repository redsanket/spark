...
public class TAppend   {
	...
	private static       int sand = 0;	// used in generation of data (in -rw and -wo test)
	...
	int     pauseIntervalMsecOption = 1000;	// pause 1 second during each loop by default
	...
	boolean openAndCloseOption = false;	// during read cycle: open and close the file during each read
	boolean newReadTheadOption = false;	// spawn new thread to read during AppendReadWrite if this option is set
	...
	boolean useFSOption = false;		// use FileSystem to do append (default is use FileContext)
	boolean useFCOption = true;			// use FileContext to do append (which is default any way)
	boolean readFullyOption = false;	// in read: if set, read every byte until the end of file to arrive at the visible length of file
	boolean alwaysHflushOption = false;	// in read: if set, read every byte until the end of file to arrive at the visible length of file
	boolean noHflushOption = false;		// in read: if set, read every byte until the end of file to arrive at the visible length of file
	boolean	readPassVEndOption = false;		// in read: if set, read until the end (else only till Visible/Written)
	boolean posReadOption = false;		// use position read if this option is set (default is sequential read)
	boolean blockLocOption = false;		// dump out the block location of the file (essentially last block)
	...
	private long visibleLenFromReadStream;		// set by the reader to be used by the writer in rw mode test
	private long byteReadFromThread;		// set by the reader of a new thread to be used by the writer in rw mode test
	
	/*
	 * Initialization & Constructor
	 */
	public TAppend() throws IOException {
		setupFS();
		initDebugInfo();
	}
	...
	public void initDebugInfo() throws IOException {
		...
		if (verboseOption)
			System.out.println("Hostname =" + hostname + "; Host IP = " + ipString );		
	}
	
	void logWrite(long tafter, long tbefore, long byteVisible, long byteWritten, long loopCount, String msg, long duration ) {
		long tdelta = tafter > tbefore ? tafter - tbefore : tbefore - tafter;
		System.out.println("TimeRW: tafter=" + tafter +   ", tbefore="   + tbefore  +    ", visible="  + byteVisible + ", written =" + byteWritten +
				", loopcount=" + loopCount + ", duration=" + duration + ", delta |tafter-tbefore|=" + tdelta + " ; " + msg);
	}

	void logRead(long tafter, long tbefore, long byteRead, long byteVisible, long loopCount, String msg, long duration) {
		long tdelta = tafter > tbefore ? tafter - tbefore : tbefore - tafter;
		System.out.println("TimeRW: tafter=" + tafter +   ", tbefore="   + tbefore  +     ", visible=" + byteVisible + ",    read="  + byteRead    +
				", loopcount=" + loopCount + ",  duration=" + duration + ", delta |tafter-tbefore|=" + tdelta + "; " + msg);
	}
	...
	static class ReadThread implements Runnable {
		...
		String 		fname;
		TAppend 	mta;	// my TAppend class handle
		FileContext readfc;
		FileSystem  readfs;
		long		byteRead;
		long		totalByteRead;
		...
		ReadThread(String s, TAppend ta) { 		// this is the file name to be open and close during each run 
			fname = s;
			mta   = ta;
			byteRead = 0;
		...
		public void run() {
			...
			try {
				...
				System.out.println("Readonly test: read visible length=" + mta.visibleLenFromReadStream +  
						" ; read beginPosition =" + beginPosition );

				byteRead = mta.readUntilEnd(in, buffer, buffer.length, mta.fnameOption, beginPosition, 
						mta.visibleLenFromReadStream, mta.readPassVEndOption, mta.posReadOption);
				mta.tsAfterRead = System.currentTimeMillis();	
				mta.byteReadFromThread = byteRead + beginPosition;
								
			} catch (IOException e){
				System.out.println("##### Caught Exception in ReadThread run(): Total byte read so far = " + totalByteRead +"; file =" + fname);
				e.printStackTrace();
			} finally {
				try {
					if (in != null)
					...
					System.out.println("##### Caught Exception in ReadThread run finally:  Total byte read so far = " + totalByteRead +"; file =" + fname);
					e.printStackTrace();		
				}
			}
			return;	
	...
	// this test always create a new thread, open and close the file. Uses only very small buffer. 
	public long readInAppendWriteInNewThread(String fname, long byteExpected) throws IOException {
		
		ReadThread rt = new ReadThread(fname, this);	// todo: add a cmdline switch to reuse the thread?
		...
		try {
			...
			System.out.println("##### Interrupted exception: testReadInAppendWriteWithNewThread. Return.");
		}
	...
	/**
	 * Layer to abstract the common file operations to use either FileContext or FileSystem
	 */
	...
	FSDataInputStream openInputStream(Path path) throws IOException {
		...
		if (useFCOption) {
			if (verboseOption) {
				System.out.println("Using FileContext to open path for read: " + path.toString());
			}
			in = mfc.open(path);
		} else {
			if (verboseOption) {
				System.out.println("Using FileSystem to open path for read: " + path.toString());
			}
			in = mfs.open(path);
		}
		return in;
	...
	public FSDataOutputStream  createAppendFile(Path path, boolean overwriteOption, boolean appendOption) throws IOException {
		if (useFCOption)
	...
	public BlockLocation[] dumpFileBlockLocations(String fname) throws IOException {
		Path path = getFullyQualifiedPath(fname);
		long filelen = getFileLengthFromFc(path);
		return dumpFileBlockLocations(path, 0, filelen);
	}

	public BlockLocation[] dumpFileBlockLocations(Path path, long start, long len) throws IOException {
		BlockLocation[] bl;
		if (useFCOption) {
		...
		System.out.println("Total Number of blocks=" + bl.length + ", block length=" + bl[0].getLength() + 
				", Host=" + bl[0].getHosts() + ", path=" + path);

		for (int i = 0; i < bl.length; i++){
			long offset = bl[i].getOffset();

			String[] names = bl[i].getNames();   // String[] topoPaths = bl[i].getTopologyPaths();
			for (String dn: names)
				System.out.println("Block index=" + i + ", DataNode=" + dn + ", Offset=" + offset + ", path=" + path );
		}
		return bl;
	}
	...
	public void createFileWithData(FileContext fc, String  name, int numBlocks,
		...
		System.out.println("  DEBUG: createFileWithData to create file: " + name + "; path = " + path.toString());
		createFileWithData(fc, path, numBlocks, CreateOpts.blockSize(blockSize), 
	...
	public int pauseCurrentThread(int millisec) throws IOException {
		try { 
			...
			System.out.println("##### Caught Interrupted Exception in pauseCurrentThread: ");
			e.printStackTrace();
		}
		return 0;
	}
	...
	public void testCreate2KB(String fname) throws IOException {
		...
		System.out.println("Checksum of the file: " + fname + " = " + mfc.getFileChecksum(path));
	}
	...
	public void testRead2KB(String fname) throws IOException {
		...
		System.out.println("Number of byte read from file: " + path.toString() + "; Byte = " + byteRead);
		in.close();
	}

	// do not read pass Visible End
	long readUntilVisibleEnd(FSDataInputStream in, byte[] buffer, long size, String fname, long pos, long visibleLen, boolean posRead) throws IOException {
		int chunkNumber = 0;
		long totalByteRead = 0;
		...
		if (verboseOption)
			System.out.println("Reader Until Visible End - begin: position: "  + pos + " ; currentOffset = " +  currentPosition + " ; bufferSize =" + buffer.length +
					 " ; VisibleLen =" + visibleLen + " ; current file Position =" + currentFpos + typeOfRead + " ; Filename = " + fname);
		...
		try { 
			while ( byteLeftToRead > 0 && currentPosition < visibleLen ) {
				...
				readParam = ", ReadParam - CurrentPostion=" + currentPosition + ", offset=0, size=" + byteToReadThisRound;
				...
				if (verboseOption) {
					System.out.println("Reader: Number of byte read="  + byteRead + "; toatlByteRead=" +  totalByteRead + "; currentPosition=" + currentPosition +
							"; chunkNumber=" + chunkNumber + "; byteLeftToRead=" + byteLeftToRead +
							"; File Pos before read=" + currentFpos + "; File name=" + fname);
			...
			throw new IOException("Exception caught in readUntilVisibleEnd (pass visibleEnd): Reader  currentOffset = " +  currentPosition + " ; totalByteRead =" + totalByteRead + 
					" ; latest byteRead = " + byteRead +   " ; visibleLen= " + visibleLen + " ; byteLeftToRead = " +  byteLeftToRead +
					" ; bufferLen = " +  buffer.length +   " ; chunkNumber= " + chunkNumber + " ; input pos = " + pos +
					" ; byteToReadThisRound = " + byteToReadThisRound +  " ; File pos before read = " + currentFpos +
					" ; Filename = " +  fname + readParam,    e );

		}
		if (verboseOption)
			System.out.println("Reader end:   position="  + pos + "; currentOffset=" +  currentPosition +"; totalByteRead =" + totalByteRead + " ; UntilVisibleEnd of Filename = " + fname);

		return totalByteRead;
	}

	// read into buffer repeatedly until the end of the file, return total number of bytes read
	// if size > buffer size, will read buffer.size one at at time (thus later data will overwrite the earlier data in buffer)

	long readUntilEnd(FSDataInputStream in, byte[] buffer, long size, String fname, long pos, long visibleLen, boolean readPassVEnd, boolean posRead) throws IOException {

		if (!readPassVEnd) { 
			return readUntilVisibleEnd(in ,buffer, size, fname, pos, visibleLen, posRead);
		}
		...
		if (verboseOption)
			System.out.println("Reader Until End - begin: position: "  + pos + " ; currentOffset = " +  currentPosition + " ; bufferSize =" + buffer.length +
					 " ; VisibleLen =" + visibleLen + " ; current file Position =" + currentFpos + typeOfRead + " ; Filename = " + fname);
		try { 
			while ( true ) {
			...
			throw new IOException("Exception caught in readUntilEnd (pass visibleEnd): Reader  currentOffset = " +  currentPosition + " ; totalByteRead =" + totalByteRead + 
					" ; latest byteRead = " + byteRead +   " ; visibleLen= " + visibleLen + " ; byteLeftToRead = " +  byteLeftToRead +
					" ; bufferLen = " +  buffer.length +   " ; chunkNumber= " + chunkNumber + " ; input pos = " + pos +
					" ; byteToReadThisRound = " + byteToReadThisRound +  " ; File pos before read = " + currentFpos +
					" ; Filename = " +  fname + readParam, e );
		...
		if (verboseOption)
			System.out.println("Reader end:   position="  + pos + "; currentOffset=" +  currentPosition + "; totalByteRead=" + totalByteRead + "; UntilEnd of Filename = " + fname);

		return totalByteRead;
	}

	// this test always open and close the file. 
	public long readInAppendWrite(String fname, long byteExpected) throws IOException {
		long totalByteRead = 0;
		...
		try {
			...
			totalByteRead = readUntilEnd(in, buffer, buffer.length, fname, beginPosition, visibleLenFromReadStream, readPassVEndOption, posReadOption );
			in.close();
			...
			System.out.println("##### Caught Exception in readInAppendWrite. Total Byte Read so far = "  + totalByteRead );
			e.printStackTrace();
			if (in != null) {
				in.close();
			}
			throw new IOException();	// let the caller knows that exception happens
		} 
	}

	/*
	 *  Write out length bytes: first 8 byte is the hex representation of the int marker
	 *  Length: >= 9, but less than 1024
	 *  TODO: if length is large, break into smaller chunk to write
	 */ 
	public long appendChunk(FSDataOutputStream out, int marker, long length, boolean flushOrNot) throws IOException
	{
		// System.out.println("WRITER: marker=" + Integer.toHexString(marker) + "; length = " + length);
		...
		// 10011001 0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\]^_`abcdefghijklmnopqrstuvwxyz0123456789:;<=>?@ABCDEFGHIJ
		// 10011002 123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\]^_`abcdefghijklmnopqrstuvwxyz0123456789:;<=>?@ABCDEFGHIJK
		int bitmask = 0x0000000F;
		for (int j = 0; j < 8; j ++) {
			int nibble = ((marker >>> (28 - 4*j)) & bitmask);
			buffer[j]  = (byte)(nibble >= 10 ? nibble - 10 + 'A' : nibble + '0');
			//System.out.println("Nibble is " + Integer.toHexString(nibble) + "; bitmask = " +
			//Integer.toHexString(bitmask) + "; buffer[j] = " + Integer.toHexString(buffer[j]));
		}
		buffer[8] = ' ';
		...
		try {
			...
			if (verboseOption)
				System.out.println("AppendChunk: Total byte written=" + totalByteWritten + "; input chunk size = " + length);

			sand++;		// next chunk will offset the data pattern by 1
			return totalByteWritten;
		} catch (IOException e){
			throw new IOException("##### IO Exception in appending file. " + out.toString(), e);
		}
	}
	
	/*
	 *  Test Append  with read and write: writer ignores the openAndClose option here
	 *  Return 0 upon success, else count of failures in negative number
	 */
	public int testAppendWriteAndRead(String fname) throws IOException {
		...
		try {
			Path path = getFullyQualifiedPath(fname);
			System.out.println("Testing testAppendWriteAndRead " + fname + "; using Filecontext: " +
					useFCOption + "; fully qualified path = " + path);
			...
			// FSDataOutputStream out = mfc.create(path, EnumSet.of(CreateFlag.APPEND, CreateFlag.CREATE));
			...
			for (int i = 0; i < ntimesOption ; i++ ){
				...
				if (toFlush){
					...
					logWrite(tsAfterHflush, tsBeforeHflush, totalByteVisible, totalByteWritten, i, "Flush " + fnameOption, tsAfterHflush - tsBeforeWrite);
				} else {
					totalByteWrittenButNotVisible += chunkSize;					
					logWrite(tsAfterHflush, tsAfterWrite, totalByteVisible, totalByteWritten, i, "NoFlush " + fnameOption, tsAfterWrite - tsBeforeWrite );
				}

				
				if (readAfterWrite) {
					byteVisibleToRead = readInAppendWrite(fname, totalByteVisible);	// testReadInAppendWrite always open and close file
					String readmsg;
					if (byteVisibleToRead >= totalByteVisible  && byteVisibleToRead <= totalByteWritten){
						readmsg = "Pass: reader sees expected size of file " + fnameOption + "  [pass]" ; // : " + byteVisibleToRead);
					} else {
						countOfFailures++;
						readmsg = "Fail: reader sees is different size of file " + fnameOption + " [fail]"; // : " + byteVisibleToRead);
					}

					logRead(tsAfterOpen4Read, tsBeforeOpen4Read, byteVisibleToRead, visibleLenFromReadStream, i, readmsg, tsAfterRead - tsAfterOpen4Read);			
				}
			...
			logWrite(tsAfterHflush, tsBeforeHflush, totalByteVisible, totalByteWritten, 0, "Close Flush " + fnameOption, now - tsBeforeWrite);	
			
			byteVisibleToRead = readInAppendWrite(fname, totalByteVisible);

			String readmsg;
			if (byteVisibleToRead == totalByteVisible){
				readmsg = "Pass: reader sees expected size of file " + fnameOption + " after close"; // : " + byteVisibleToRead);
			} else {
				countOfFailures++;
				readmsg = "Fail: reader sees is different size of file " + fnameOption + " after close" ; // : " + byteVisibleToRead);
			}
			long lenFromFc = getFileLengthFromFc(path);
			logRead(tsAfterOpen4Read, tsBeforeOpen4Read, byteVisibleToRead, lenFromFc, 0, readmsg, tsAfterRead - tsAfterOpen4Read);			
	...
	public FSDataOutputStream  createAppendFileFC(Path path, boolean overwriteOption, boolean appendOption) throws IOException {
		FSDataOutputStream out;

		// if exist, do OVERWRITE or APPEND based on -overwrite flag), else create it with CREATE|APPEND 
		if (ifExists(path)) {
			if (overwriteOption) {
				System.out.println("File already exists. Truncate file. FileContext Open OVERWRITE: " + path);
				out = mfc.create(path, EnumSet.of( CreateFlag.OVERWRITE));
			} else if (appendOption) {  
				System.out.println("File already exists. FileContext Open with APPEND only: " + path);
				out = mfc.create(path, EnumSet.of(  CreateFlag.APPEND));
			} else {
				System.out.println("File exist, but neither overwrite nor append is specified. Aborted." + path);
				throw new IllegalArgumentException();	
			}
		} else {
			// System.out.println("File does not exists. Open with CREATE + APPEND: " + path);
			// out = mfc.create(path, EnumSet.of( CreateFlag.APPEND,  CreateFlag.CREATE));
			System.out.println("File does not exists. FileContext Open with CREATE: " + path);
	...
	// the new semantics:
	// CREATE: can work with CREATE|APPEND, and CREATE|OVERWRITE (TRUNC)
	public FSDataOutputStream  createAppendFileFS(Path path, boolean overwriteOption, boolean appendOption) throws IOException {
		FSDataOutputStream out;

		// if exist, do OVERWRITE or APPEND based on -overwrite flag), else create it with CREATE|APPEND 
		if (ifExists(path)) {
			if (overwriteOption) {
				System.out.println("File already exists. Truncate file. FileSystem Open OVERWRITE: " + path);
				out = mfs.create(path, overwriteOption);
			} else if (appendOption) {  
				System.out.println("File already exists. FileSystem Open with APPEND  (mfs.append) only: " + path);
				out = mfs.append(path);
			} else {
				System.out.println("File exist, but neither overwrite nor append is specified. Aborted." + path);
				throw new IllegalArgumentException();	
			...
			// System.out.println("File does not exists. Open with CREATE + APPEND: " + path);
			// out = mfc.create(path, EnumSet.of( CreateFlag.APPEND,  CreateFlag.CREATE));
			System.out.println("File does not exists. FileSystem Open with CREATE (mfs.createNewFile): " + path);
			out = mfs.create(path, overwriteOption);
		}
		return out;
	}
	...
	public void dumpOptions( ) {
		if (verboseOption) {
			...
			System.out.println("INFO: bothReadWriteOption " + bothReadWriteOption);
			...
			System.out.println("INFO: pauseIntervalMsecOption " + pauseIntervalMsecOption);
			System.out.println("INFO: ntimesOption " + ntimesOption);
			System.out.println("INFO: chunkSizeOption " + chunkSizeOption);
			System.out.println("INFO: openAndCloseOption " + openAndCloseOption);
			...
			System.out.println("INFO: alwaysHflushOption " + alwaysHflushOption);
			System.out.println("INFO: noHflushOption " + noHflushOption);
			System.out.println("INFO: readPassVEndOption " + readPassVEndOption);
	...
	public  void getCmdlineOptions(String[ ] args) {  	
		...
		for (int i = 0; i < args.length; i++) {
			...
			} else if (args[i].equals("-ro")) { 	// options -ro, -rw, -wo are mutually exclusive.
				readOnlyOption = true;
			...
			} else if (args[i].equals("-useFS")) {	// -useFS and -useFC are mutually exclusive
				useFSOption = true;
				useFCOption = false;
			} else if (args[i].equals("-alwaysHflush")) {	// -always Hflush and no Hflush are mutualluy exclusive
	...
	public  int testReadWrite(){
		...
		try {
			// setupFS();
			errorStat = testAppendWriteAndRead(fnameOption); // test both read and write in a loop
		} catch (Exception e) {
			errorStat = -1;
	...
	// To read every byte of a file could be slow. One way is to use binary search to probe the end of a file
	// The other way is to get to the end of file from DFSDataInputStream Thus the strategy is to:
	...
	public  int  testReadOnly(){
		...
		System.out.println("testReadOnly to file " + fnameOption + " ; for " + ntimesOption + " times with pause " + pauseIntervalMsecOption);
		...
		try {		
			...
			Path path = getFullyQualifiedPath(fname);
			System.out.println("Testing testReadOnly " + fname + "; fully qualified path = " + path);
			long lenFromFc = getFileLengthFromFc(path);
			
			if (blockLocOption){
				BlockLocation[] bl = dumpFileBlockLocations(path, beginPosition, lenFromFc);
			}
			...
			for (int i = 0 ; i < ntimesOption; i++ ) {
				if ( in == null  || openAndCloseOption ) {
					...
					if (readFullyOption) {
						...
						beginPosition = knownVisibleLength < 1000 ? 0 : knownVisibleLength - 1000;
					}		
					
					totalByteSeenHere = beginPosition;
					System.out.println("Readonly test: read visible length=" + knownVisibleLength +  
							" ; read beginPosition =" + beginPosition + "; iteration = " + i);				
				}
	
				//  does not re-seek. 
				byteRead = readUntilEnd(in, buffer, buffer.length, fnameOption, beginPosition, knownVisibleLength, readPassVEndOption, posReadOption);
				...
				String readmsg =  " test readonly " + fnameOption + "; current lenFromFc =" + Long.toString(lenFromFc); 
				logRead(tsAfterOpen4Read, tsBeforeOpen4Read, totalByteSeenHere, knownVisibleLength, i, readmsg, duration);			
			...
			errorStat = -2;
			System.out.println("##### Caught Exception in testReadOnly while reading.");
			e.printStackTrace();
			return errorStat;
		}
		System.out.println("READER: testReadOnly total byte read: " + totalByteRead +
				"; after iterations = " + ntimesOption);
	...
	// open file in OVERWRITE|APPEND mode, then in a loop to do N time: write a chunk, pause
	...
	public int  testWriteOnly(){
		...
		System.out.println("testWriteOnly to file " + fnameOption + " ; for " + numBlocks + " times with pause " + pauseIntervalMsecOption);

		try {

			Path path = getFullyQualifiedPath(fname);
			System.out.println("Testing testWriteOnly " + fname + "; fully qualified path = " + path);
			...
			long fpos = out.getPos();	// data may already exist, if open in append mode; but getPos() does not work for append mode

			System.out.println("File original length =" + originalFileVisibleLength +" ; getPos returned: " + fpos +
					"; overWriteOption = " + overwriteOption + "; appendOption =" + appendOption );

			if (blockLocOption){
				BlockLocation[] bl = dumpFileBlockLocations(path, fpos > 1000 ? fpos - 1000 : 0, chunkSize);
			}
			...
			for (int i = 0; i < ntimesOption ; i++ ){
				...
				if (toFlush) {
					totalByteVisible += chunkSize + totalByteWrittenButNotVisible ;
					totalByteWrittenButNotVisible = 0;
					logWrite(tsAfterHflush, tsBeforeHflush, totalByteVisible, totalByteWritten, i, fnameOption, tsAfterHflush - tsBeforeWrite);
				} else {
					totalByteWrittenButNotVisible += chunkSize;					
					logWrite(tsAfterHflush, tsAfterWrite, totalByteVisible, totalByteWritten, i, fnameOption, tsAfterWrite - tsBeforeWrite );
				}
			...
			stat = -1;
			System.out.println("##### ERROR: testWriteOnly: Caught Exception in writing to file: "); 
			e.printStackTrace();
		} finally {
			if (out != null)
				try {
					...
					System.out.println("##### Exception encountered during closing of file. Ignored.");
				}
		}
		return (stat);
	}
	...
	public  int testRWSanity() {
		...
		try {
			// test the creation of file, and write 2K worth of data, close, then read it to verify the size
	...
	public int testReadBeyondEnd() throws IOException {
		...
		System.out.println("Test to test the behavior when read from beyond the file length. File name = " + fnameOption) ;
		...
		long beginPosition = 0;
		long byteRead = readUntilEnd(in, buffer, buffer.length, fnameOption, beginPosition, -1, true, true);	// position read
		System.out.println("Read from begin to end of file: number of byte read in =  " + byteRead);
		

		// read 100 byte from end of file
		beginPosition = fileLen - 100;
		byteRead = readUntilEnd(in, buffer, buffer.length, fnameOption, beginPosition, -1, true, true);	
		System.out.println("Read file 100 byte from end: number of byte read in =  " + byteRead);
		
		// read from exactly the end of the file
		byteRead = readUntilEnd(in, buffer, buffer.length, fnameOption, fileLen, -1, true, true);	
		System.out.println("Read file from. End number of byte read in =  " + byteRead);
		
		// read 200 byte beyond the end of the file
		byteRead = readUntilEnd(in, buffer, buffer.length, fnameOption, fileLen + 200, -1, true, true);	
		System.out.println("Read file from End. number of byte read in =  " + byteRead);
	...
	public int testSeekSeqRead() throws IOException {
		...
		try {
			...
			fpos = in.getPos();
			System.out.println("After read of " + byteRead + " byte, getPos() returns: " + fpos);
			byteRead = in.read(buffer, 0, 200);
			fpos = in.getPos();
			System.out.println("After read of " + byteRead + " byte, getPos() returns: " + fpos);
			
			System.out.println("Now Seek file to 1000 from the end where total length of file = " + filelen);
			in.seek(filelen - 1000 );
			fpos = in.getPos();
			System.out.println("getPos() returns: " + fpos);

			System.out.println("Now Seek file to exactly end of file: " + filelen);
			in.seek(filelen );
			fpos = in.getPos();
			System.out.println("getPos() returns: " + fpos);

			System.out.println("Now Seek file to 100000 beyond end of file. Expect Exception.");
			...
			fpos = in.getPos();
			System.out.println("##### Caught Expected Exception in testSeek: getPos =" + fpos);
			return 0;
		}	
...
*/			
			...
			if (stat == 0 ) {
				...
				System.out.println("Tests failed, status = " + stat + " ; Dump out Block Location");
