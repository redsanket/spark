...
junk() {
	if [ -n "$HDFT_HADOOP_ENV_SH" ] ; then
		echo from util: source "${HDFT_HOME}/src/hdft_hadoop_env.sh"
		source "${HDFT_HOME}/src/hdft_hadoop_env.sh"
	fi
}
...
# export HDFT_JOBS="DFSIO NNBench Scan Shuffle SmallJobs Sort"

# export HDFT_JOBS="NNBench Scan Shuffle SmallJobs Sort"
# export HDFT_RESULT=0
# export HDFT_JOBS_SCRIPTS_TOP_DIR=$SG_WORKSPACE/job_scripts/
# export ARTIFACTS=$SG_WORKSPACE/artifacts
...
function hdftKInitOld {
	...
	"haoopqa")
		echo "    kinit -c  /tmp/hdft_kbr012345 -kt ~/hadoopqa.dev.headless.keytab $*   hadoopqa"
		      kinit -c  /tmp/hdft_kbr012345 -kt ~/hadoopqa.dev.headless.keytab  $*   hadoopqa
		;;
	"hdfs" | "hadoop9" )
		export EFFECTIVE_KB_USER="hdfs"
		...
		;;
	"hdfsqa" )
		export EFFECTIVE_KB_USER="hdfsqa"
		echo "    kinit -k -t /homes/hdfsqa/etc/keytabs/hdfsqa.dev.service.keytab $*   hdfsqa@DEV.YGRID.YAHOO.COM"
		          kinit -k -t /homes/hdfsqa/etc/keytabs/hdfsqa.dev.service.keytab $*   hdfsqa@DEV.YGRID.YAHOO.COM
		;;
	"hadoop8" | "cwchung"  )
		echo "kinit -k -t ~hadoop8/hadoop8.dev.headless.keytab $* hadoop8@DEV.YGRID.YAHOO.COM"
		kinit -k -t ~hadoop8/hadoop8.dev.headless.keytab $* hadoop8@DEV.YGRID.YAHOO.COM
		;;
	*)
		export EFFECTIVE_KB_USER="hadoopqa"
		echo "    kinit -c  /tmp/hdft_kbr012345 -kt ~/hadoopqa.dev.headless.keytab $*   hadoopqa"
...
function displayEntryMessage {
  dumpmsg "***************************************************************************************************************************************************************************"
  dumpmsg "BEGINNING TO EXECUTE  $HDFT_JOBS ON $CLUSTER CLUSTER at `date`"
  dumpmsg "***************************************************************************************************************************************************************************"
}
...
function hdftShowStderrResult {
    ...
    if [ "$stat" == 0 ] ; then
	dumpmsg "    [Pass - $taskid]: OK  in xact [subtask cker $JOB $taskid - pass test: $passmsg; got stderr string: $stderrPattern]"
    else
	dumpmsg "    [Fail - $taskid]: BAD in xact [subtask cker $JOB $taskid - fail test: $failmsg; did not get stderr string: $stderrPattern]"
    fi
}
...
function hdftShowExecResult {
    ...
    if [ "$stat" == 0 ] ; then
		dumpmsg "    [Pass - $taskid]: OK  in xact [subtask exec ${JOB}- $passmsg]"
    else
		dumpmsg "    [Fail - $taskid]: BAD in xact [subtask exec ${JOB}- $failmsg]"
    fi
}
...
function hdftDiffMd5 {
    ...
    if  [ "$md5Expected" == "$md5Actual" ] ; then
		dumpmsg "    [Pass - $taskid]: OK  in xact [subtask data $JOB - pass test: md5sum matches expected $md5Expected. $passmsg]"
		returnStat=0
    else
		dumpmsg "    [Fail - $taskid]: BAD in xact [subtask data $JOB - fail test: md5sum mismatch: expected $md5Expected. Got $md5Actual. $failmsg]"
		returnStat=1
    fi
	return $returnStat
}
...
function hdftShowDataResult {
    ...
    if [ "$stat" == 0 ] ; then
		dumpmsg "    [Pass - $taskid]: OK  in xact [subtask data $JOB - pass test: $patternPassMsg $passmsg]"
    else
		dumpmsg "    [Fail - $taskid]: BAD in xact [subtask data $JOB - fail test: $patternFailMsg $failmsg]"
    fi
}
...
function hdftShowDiffResult {
    ...
    if [ "$stat" == 0 ] ; then
		dumpmsg "    [Pass - $taskid]:  OK  in xact [diff $JOB $taskid - Pass test: $passmsg]"
		#dumpmsg "Pass in [$taskid - diff]:  OK  in $JOB subtask diff - [pass $taskid $passmsg]"
    else
	case $stat in 
	1)
		dumpmsg "    [Fail - $taskid]:  BAD in xact [subtask diff $JOB $taskid - fail test: $passmsg: output file mismatch $file1  $file2]"
		;;
	2)
		dumpmsg "    [Fail - $taskid]:  BAD in xact [subtask diff $JOB $taskid - fail test: $passmsg: file1 does not exist $file1]"
		;;
	3)
		dumpmsg "    [Fail - $taskid]:  BAD in xact [subtask diff $JOB $taskid - fail test: $passmsg: file2 does not exist $file2]"
		;;
	*)	
		dumpmsg "    [Fail - $taskid]:  BAD in xact [subtask diff $JOB $taskid - fail test: $passmsg: unknown error $file1  $file2]"
		;;
	esac
    fi
}
...
function hdftShowTaskResult {
    ...
    if [ "$stat" == 0 ] ; then
		dumpmsg "    [PASS - $taskid]: OK  in XACT [run  $JOB $taskid - pass test: $passmsg]"
		dumpmsg "#TK#|PASS | $taskid| $cmdmsg | $passmsg |"

    else
		dumpmsg "    [FAIL - $taskid]: BAD in XACT [run  $JOB $taskid - fail test: $failmsg]"
		dumpmsg "#TK#|FAIL | $taskid| $cmdmsg | $failmsg |"
    fi
	$HDFT_TOP_DIR/hdft_reportStatus.sh "$stat"  "$taskid" "$passmsg" "$failmsg"
...
function hdftShowBriefTaskResult {
    ...
    if [ "$stat" == 0 ] ; then
		dumpmsg "    [PASS - $taskid]: OK  in XACT [$cmdmsg] [$passmsg]"
		dumpmsg "#TK#|PASS | $taskid| $cmdmsg | $passmsg |"
    else
		dumpmsg "    [FAIL - $taskid]: BAD in XACT [$cmdmsg] [$failmsg]"
		dumpmsg "#TK#|FAIL | $taskid| $cmdmsg | $failmsg |"
    fi
...
function hdftDisplayTestGroupMsg {
		local msg=$1		## PARAM
		dumpmsg "############################### Test Group: $msg ##############################################"
		dumpmsg "############################### Test Group: $msg ##############################################"
		dumpmsg "#TH#|*Status* | *TestId*| *Test Command for ${msg}*| *Test Notes* |"

}

###########################################
# Function to check the result of the test run
# checkResult  - subroutine input param: env variable $HDFT_RESULT
###########################################
function checkResult {
  (( HDFT_RESULT = $HDFT_RESULT + $1 ))
  if [ "$1" -eq 0 ] ; then
    displaySuccess
  else
    displayFailure
...
# displayFailure  - subroutine input param: env variable $HDFT_JOBS $HDFSSEC_ARTIFACTS_DIR
###########################################
function displayFailure {
  displayHeader "Execution of $HDFT_JOB FAILED. Please see $HDFSSEC_ARTIFACTS_DIR/$HDFT_JOB.log  for more details"
}
...
# displaySuccess  - subroutine input param: env variable $HDFT_JOB
###########################################
function displaySuccess {
  displayHeader "Execution of $HDFT_JOB PASSED"
}

###########################################
# Function to delete artifacts directory
# deleteArtifacts  - subroutine input param: env variable $ARTIFACTS
###########################################
function makeArtifactsDir {
  if [ -z "${HDFL_ARTIFACTS_TOP_DIR}" ] ; then
	echo "ERROR in config: key env variable HDFL_ARTIFACTS_TOP_DIR is not set"
	exit 199
  fi

  if [ -d ${HDFL_ARTIFACTS_TOP_DIR} ] ; then
    echo ""
    # rm -rf ${HDFL_ARTIFACTS_TOP_DIR}
  fi

  if [ ! -d ${HDFL_ARTIFACTS_TOP_DIR} ] ; then
    mkdir -p ${HDFL_ARTIFACTS_TOP_DIR}
  fi
...
# createLocalDir  - subroutine input param: $1 dirname
###########################################
...
# deleteLocalDir  - subroutine input param: $1 dirname
###########################################
...
###########################################
# Function to create a directory if not exist on HDFS 
# createHdfsDir  - subroutine input param: $1 dirname. Also use env HADOOP_COMMON_HOME, HADOOP_CONF_DIR
###########################################
function createHdfsDir {
  DIR=$1		## PARAM
  echo "$DFS_CMDs --config $HADOOP_CONF_DIR dfs -stat $DIR"
  $DFS_CMDs --config $HADOOP_CONF_DIR dfs -stat $DIR
  if [ "$?" -ne 0 ]; then
    echo "$DFS_CMD  --config $HADOOP_CONF_DIR dfs -mkdir -p $DIR"
    $DFS_CMD  --config $HADOOP_CONF_DIR dfs -mkdir -p $DIR
  fi
}

###########################################
# Function to delete a directory on HDFS
# deleteHdfsDir  - subroutine input param: $1 dirname. Also use env HADOOP_COMMON_HOME, HADOOP_CONF_DIR
###########################################
function deleteHdfsDir {
  DIR=$1		## PARAM
  echo "$DFS_CMD --config $HADOOP_CONF_DIR dfs -stat $DIR"
  $DFS_CMD --config $HADOOP_CONF_DIR dfs -stat $DIR
  if [ "$?" -eq 0 ]; then
    echo "$DFS_CMD --config $HADOOP_CONF_DIR dfs -rmr -skipTrash $DIR"
    $DFS_CMD --config $HADOOP_CONF_DIR dfs -rmr -skipTrash $DIR
  fi
}
...
# setTestCaseDescription  - subroutine input param: $1 id, $2 name, $3 description
###########################################
function setTestCaseDescription {
	id=$1;  name=$2;  desc=$3;		## PARAM
	export TEST_CASE_DESC="$1 $2 - $3"
}
###########################################
# Sets build description
# setBuildDescription  - subroutine input param: none.  Use env HADOOP_COMMON_HOME, HADOOP_CONF_DIR, CLUTER, HDFT_JOB
###########################################
function setBuildDescription {
    if [ -n "HADOOP_VERSION" ] && [ "$HADOOP_VERSION" == "20" ] ; then
	VERSION=`hadoop version`
    else
	VERSION=`$HADOOP_COMMON_HOME/bin/hadoop  --config $HADOOP_CONF_DIR version | head -1`
    fi
    echo "Ran Job $HDFT_JOBS on Hadoop Version $VERSION, on cluster $CLUSTER `date`"
}

###########################################
# A thin wrapper to do the exec. Original idea was to log additional info in this routine, but turns out can't 
# hdftExecHadoopDfsCmd  - subroutine input param: any and everything
###########################################
function hdftExecHadoopDfsCmd {
	HadoopCmd="$HDFT_HADOOP_DFS_CMD $*"
	$HadoopCmd
	stat=$?
	return $stat
}
...
function hdftDisplayTasksStatus {
	...
	if [ "$stat" == 0 ] ; then
	   dumpmsg "____XACT____ TASKS PASSED: $taskName"
	else
	   dumpmsg "____XACT____ TASKS FAILED: $taskName. Result=$stati"	
	fi
}

###########################################
# Given full or relative path containing job_scripts directory, extract the relative pathname of job dir 
# this is used for generating default output and expected file name
###########################################
function hdftGetDefaultRelOutputDir {
	my_progSh=$1 ; 		## PARAM
	my_prog=`basename $my_progSh .sh`
	my_progDir=`dirname $my_progSh`
	my_jobDir=`echo $my_progDir | sed -e 's#[-/DA-Za-z0-9_\.]*job_scripts/##' `

	export HDFT_JOB_DEFAULT_REL_OUTPUT_DIR="$my_jobDir/$my_prog"

	echo "my_progSh=$my_progSh"
	echo "my_prog=$my_prog"
	echo "my_progDir=$my_progDir"
	echo "my_jobDir=$my_jobDir"
	echo "HDFT_JOB_DEFAULT_REL_OUTPUT_DIR=$HDFT_JOB_DEFAULT_REL_OUTPUT_DIR"
}
...
function hdftFilterOutput {
	...
	dumpmsg "    hdftFilterOutput -- type=$type; mv $file $TFILE "
	mv $file $TFILE
	...
	"std" | *)
		dumpmsg "    hdftFilterOutput -- sed -f ${HDFT_TOP_DIR}/src/hdft_date.sed $TFILE | fgrep -v -f ${HDFT_TOP_DIR}/src/etc/grep-v.txt \> $file"
...
function hdftDoDiff {
	...
	if [ -n "HDFT_VERBOSE" ] && [ "HDFT_VERBOSE" == 1 ] ; then
		dumpmsg  "    === do diff $file1 $file2"
	fi
	if [ -z "$file1" ] || [ ! -f "$file1" ] ; then
	    dumpmsg "    error in hdftDoDiff: first file [$file1] does not exist"
	    stat=2
	fi
	if [ -z "$file2" ] || [ ! -f "$file2" ] ; then
	    dumpmsg "    error in hdftDoDiff: second file [$file2] does not exist"
	    stat=3
	fi
	if [ $stat != 0  ]; then
		return $stat
	...
	# expected results in file2 contains gateway address
	# so the test failed if it is run on a different machine
	# I added this quick fix to make this info always match the current gateway
	localmachine=`hostname`
	file3=${file2}-tmp
	cat $file2 | sed -e "s/gwbl2005.blue.ygrid.yahoo.com/${localmachine}/g" > ${file3}
	...
	stat=$?
	if [ $stat != 0 ] ; then
		diff $file1 $file3 | head -10 | cat -n
	fi
...
function hdftMapStatus {
	...
	case "$maptype" in 
	"neg"|"NEG"|"kbneg")			# reverse the status for negative test. 
		if [ $stat == 0 ] ; then
			return 1
		else
			return 0
...
###########################################
# getting the NN from the HADOOP_CONF_DIR directory. Not all that reliable.
###########################################
function hdft_getNN_V22 {
        ...
        "0"|""|"default")
                NN=`echo $NN_List | awk -F, '{print $1}' `
                ;;
        "1")
                NN=`echo $NN_List | awk -F, '{print $2}' `
                ;;
        "2")
                NN=`echo $NN_List | awk -F, '{print $3}' `
                ;;
        *)
                NN=`echo $NN_List | awk -F, '{print $1}' `
                ;;
