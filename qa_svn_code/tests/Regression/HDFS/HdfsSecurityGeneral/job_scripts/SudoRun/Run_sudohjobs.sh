...
execOneHadoopJob() {
	...
	*)
	...
	$HADOOP_CMD dfs -cat $HQ/tmpWrite/Basic/run_hjob/wordcount-out/* | sort -k2 -nr
	echo "hdfs dfs -cat $TASK_OPERAND_DEST/'*' | sort -k2 -nr > ${TASK_OUTPUT_ART} "
	...
	hdftShowTaskResult "$LOCAL_TASKS_RESULT" "$msg" "$CORETEST_OP"
...
# 	execOneHadoopJob "SF022" "99"    wordcount    default default  hdfsTestData/basic/smallFiles  run_hjob/wordcount-out
	echo "   Run_sudohjobs- Warning: KB superuser are not allowed to run mapred jobs."
	echo "   Run_sudohjobs- Warning: KB superuser are not allowed to run mapred jobs."
}

execAllTests() {
#	execOneHadoopJob "SF022" "9"    wordcount    default default  hdfsTestData/basic/smallFiles  run_hjob/wordcount-out
#	execOneHadoopJob "SF022" "99"    wordcount    default default  hdfsTestData/basic/smallFiles  default
	echo "   Run_sudohjobs- Warning: KB superuser are not allowed to run mapred jobs."
	echo "   Run_sudohjobs- Warning: KB superuser are not allowed to run mapred jobs."
}
