...
HDFD_DEFAULT_PERM_DIR=$HDFD_TOP_DIR/hdfsTestData/PermTest/
...
execOnePermJob() {
	...
	"file_create" | "file_write" )
		...
		# for f in $FLIST ; do 
			# $HADOOP_CMD dfs -touchz $f  >>  $TASK_OUTPUT_ART  2>> $TASK_OUTPUT_ART
 
			# stat=$?
			# echo "    hdfs dfs -touchz $f return $stat"
		# done

		NLINE_PERM_DENIED=`cat  $TASK_OUTPUT_ART | fgrep 'Permission denied' | grep 'hdfsqa' | grep '7[046][046]' | wc -l`
		NLINE_IN_FILE=`cat  $TASK_OUTPUT_ART  | grep -v -f "$HDFT_TOP_DIR/src/etc/grep-v.txt" | wc -l `
		echo "    Output file has NLINE_PERM_DENIED=$NLINE_PERM_DENIED lines of Permission Denied, out of NLINE_IN_FILE=$NLINE_IN_FILE total lines"

		if [ $TASK_TEST_TYPE == "MIXED" ] ; then
			# ugly way of ascertain if the run is successful
			if [ $NLINE_IN_FILE == 6 ] && [ $NLINE_PERM_DENIED == 6 ] ; then
			    statExec=0
			else
			   statExec=1
			fi
		else	# positive test case: should have no Permission Denied
			if  [ $NLINE_PERM_DENIED == 0 ] && [ $NLINE_IN_FILE == 0 ] ; then
			   statExec=0
			else
			   statExec=1
			fi
