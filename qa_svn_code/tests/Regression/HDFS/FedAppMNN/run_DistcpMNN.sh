...
###################################################
## Exercise distcp on multiple NNs within a cluster
...
function setup {
	...
	TIDENT="FED_DCPY"	# Ident of this entire set of tests FED_DCPY, _101, ...
	...
	MD5SUM_WORDCOUNT_INPUT_ALL1="02ad8298a3c6e422f72489ac16c01637  -"
	MD5SUM_WORDCOUNT_INPUT_ALL2="77f90eeb2a5f32a78225a6b66ed98feb  -"
...
function teardown {
	...
	for nn in $NAMENODES; do
		...
		deleteHdfsDir $FED_WC_INDIR_SLASH hdfs://$nn 0
...
### Param1: fully qualitifed output directory. Return 0 if match
function verifyDistcpOutput {
	echo "verifyDistcpOutput   OUTPUT_DIR=$OUTPUT_DIR "
	echo "Expect to see 3 files with match md5sum for cat of all 3"
	OUTPUT_DIR=$1
	DISTCP_OUTPUT_DIR=${OUTPUT_DIR}
	local cmd="hdfs dfs -test -d  $DISTCP_OUTPUT_DIR"
	echo "    UCmd1: $cmd"
	eval $cmd

	if [ $? != 0 ]; then
		setFailCase  "VerifyDistcpOutput:: Error File/Dir  $DISTCP_OUTPUT_DIR cannot be found"
		hdftReturnTaskResult 1 $TID "VerifyDistcpOutput:: Error File/Dir  $DISTCP_OUTPUT_DIR cannot be found"
        return 1
	fi


	echo "hdfs dfs -cat $DISTCP_OUTPUT_DIR/* \ md5sum \ cut -c1-32 "
	cmd="hdfs dfs -cat $DISTCP_OUTPUT_DIR/* | md5sum | cut -c1-32 "
	echo "    #### Ucmd2: $cmd"
	eval $cmd   > $TMPDIR/DC_${TID}.tmp  2>$TMPDIR/DC_${TID}.err
	OUTPUT_MD5=`cat < $TMPDIR/DC_${TID}.tmp`
	#OUTPUT_MD5=`hdfs dfs -cat $DISTCP_OUTPUT_DIR/* | md5sum | cut -c1-32 `

	# Note the use of MD5SUM_WORDCOUNT_INPUT_ALL, not _OUTPUT
	if [ $MD5SUM_WORDCOUNT_INPUT_ALL1  == $OUTPUT_MD5 ] || [ $MD5SUM_WORDCOUNT_INPUT_ALL2  == $OUTPUT_MD5 ] ; then
		hdftReturnTaskResult 0 $TID "Distcp Pass"
		return 0
	else
		setFailCase  "VerifyDistcpOutput:: Mismatch. Expected: $MD5SUM_WORDCOUNT_INPUT_ALL1 ; Got $OUTPUT_MD5"
		hdftReturnTaskResult 1 $TID "VerifyDistcpOutput:: Error File/Dir  $DISTCP_OUTPUT_DIR cannot be found"
		${MY_HDFS_CMD}  dfs -lsr $DISTCP_OUTPUT_DIR
		return 1
	fi
}
...
#                     $1     $2                           $3                          $4                                    $5                           $6
#	run_distcp 	"100" "DEFAULT"                      $FED_WC_INDIR_REL     $FED_WC_OUTDIR_REL	     "Default NN-home src to default NN-home dest"    "option to distcp"
# MAPRED_OPTION=-Dmapreduce.job.hdfs-servers=hdfs://gsbl90773.blue.ygrid.yahoo.com:8020
function run_distcp {

	TID="${TIDENT}_${1}"	# testId
	MY_INFS=${2}			# fully qualified input Dir of where teragen output the data
	MY_INDIR=${3}			# fully qualified input Dir of where teragen output the data
	...
	echo "    #### [$PROGNAME - test_run_distcp] param: $*"
	echo "    #### [$PROGNAME - test_run_distcp] param: TID=$TID"
	echo "    #### [$PROGNAME - test_run_distcp] param: MY_INFS=$MY_INFS"
	echo "    #### [$PROGNAME - test_run_distcp] param: MY_INDIR=$MY_INDIR"
	echo "    #### [$PROGNAME - test_run_distcp] param: MY_OUTDIR=$MY_OUTDIR"
	echo "    #### [$PROGNAME - test_run_distcp] param: TOPTION=$TOPTION"
	echo "    #### [$PROGNAME - test_run_distcp] param: JOB_OPTION=$JOB_OPTION"
	echo "    #### [$PROGNAME - test_run_distcp] param: MY_INDIR_FQ=$MY_INDIR_FQ"
	echo "    #### [$PROGNAME - test_run_distcp] param: TESTCASE_DESC=$TESTCASE_DESC"
	...
	local 	cmd="${MY_HADOOP_CMD} distcp  $JOB_OPTION  $TOPTION $MY_INDIR_FQ  $MY_OUTDIR"
	echo "    #### TCMD1=$cmd"
	eval $cmd
	if [ $? != 0 ] ; then
	    setFailCase "FAILED in exec distcp [3]. [$cmd]"
	    hdftReturnTaskResult 1 $TID "$cmd" "distcp "
		return 1
	fi

	
	${MY_HDFS_CMD} dfs -test -d $MY_OUTDIR	# test -d return 0 if directory exists
	if [ $? != 0 ] ; then
	    setFailCase "FAILED to create distcp output dir [4]. [$cmd]"
	    hdftReturnTaskResult 1  $TID "$cmd" "distcp "
		return 1
	fi
	...
	cmd="${MY_HDFS_CMD} dfs -cat $MY_OUTDIR/* | md5sum "
	echo "    #### UCmd3: $cmd "
	OUTPUT_MD5=`eval $cmd`
	
	if [ $? != 0 ] ;then
	    setFailCase "FAILED to cat  distcp output file to md5sum [6]."
	    hdftReturnTaskResult 1  $TID "$cmd" "FAILED to cat  distcp output file to md5sum"
		return 1
	fi
	...
	if [ "$MD5SUM_WORDCOUNT_INPUT_ALL1"  == "$OUTPUT_MD5" ] || [ "$MD5SUM_WORDCOUNT_INPUT_ALL2"  == "$OUTPUT_MD5" ] ; then
	    ...
	    hdftReturnTaskResult 0  $TID "$cmd" "distcp PASS"
        return 0
    else
	    setFailCase "FAILED: MD5sum of distcp output is $OUTPUT_MD5. Expected $MD5SUM_WORDCOUNT_INPUT_ALL [7]."
	    hdftReturnTaskResult 1  $TID "$cmd" "FAILED: MD5sum of distcp output is $OUTPUT_MD5. Expected $MD5SUM_WORDCOUNT_INPUT_ALL"
		return 1
    fi
...
# Notes that -fs is not supported, as with mapred applications (hdfs understoods it, but mapred failed with ( javax.security.auth.Subject.doAs(Subject.java:396)
function do_test1 {
	#                $1     $2                            $3                         $4                                          $5 		      $6 
    #                ID    fs of src                      dir of src            dir of dest                                     descriptions	special option
	hdftDisplayTestGroupMsg "Test distcp across multiple NNs, with different URI. Src from default host NNX/NN0. Output to same defaul thost"		""
	run_distcp 	"100" "DEFAULT"                      $FED_WC_INDIR_REL     $FED_WC_OUTDIR_REL	     "Default NN-home src to default NN-home dest"		""

	run_distcp 	"110" "DEFAULT"                      $FED_WC_INDIR_SLASH   $FED_WC_OUTDIR_SLASH	     "Default NN src to default NN dest"		""
	run_distcp 	"120" "hdfs://$NN0.blue.ygrid"       $FED_WC_INDIR_SLASH   "hdfs://$NN0.blue.ygrid/$FED_WC_OUTDIR_SLASH"    "FQ hdfs default NN src to FQ default NN dest"		""
	run_distcp 	"160" "hftp://$NN0.blue.ygrid:50070" $FED_WC_INDIR_SLASH   "hdfs://$NN0.blue.ygrid/$FED_WC_OUTDIR_SLASH"    "FQ hftp NN1 src to FQ NN2 dest"		""

	hdftDisplayTestGroupMsg "Test distcp across multiple NNs, with different URI. Src from NN1 or NN2. Output to same host."
	run_distcp 	"200" "hdfs://$NN1.blue.ygrid"       $FED_WC_INDIR_SLASH   "hdfs://$NN1.blue.ygrid/$FED_WC_OUTDIR_SLASH"    "FQ hdfs default NN src to FQ default NN dest"		""
	run_distcp 	"230" "hftp://$NN1.blue.ygrid:50070" $FED_WC_INDIR_SLASH   "hdfs://$NN1.blue.ygrid/$FED_WC_OUTDIR_SLASH"    "FQ hftp NN1 src to FQ NN2 dest"		""
	run_distcp 	"250" "hdfs://$NN2.blue.ygrid"       $FED_WC_INDIR_SLASH   "hdfs://$NN2.blue.ygrid/$FED_WC_OUTDIR_SLASH"    "FQ hdfs default NN src to FQ default NN dest"		""
	run_distcp 	"270" "hftp://$NN2.blue.ygrid:50070" $FED_WC_INDIR_SLASH   "hdfs://$NN2.blue.ygrid/$FED_WC_OUTDIR_SLASH"    "FQ hftp NN1 src to FQ NN2 dest"		""

	hdftDisplayTestGroupMsg "Test distcp across multiple NNs, with different URI. Src from default host NNX/NN0. Output to NN1/NN2"		"-i"
	run_distcp 	"300" "DEFAULT"                      $FED_WC_INDIR_REL     "hdfs://$NN1.blue.ygrid/$FED_WC_OUTDIR_SLASH"    "Default NN-home src to default NN-home dest"		"-i"
	run_distcp 	"310" "DEFAULT"                      $FED_WC_INDIR_SLASH   "hdfs://$NN1.blue.ygrid/$FED_WC_OUTDIR_SLASH"    "Default NN src to default NN dest"		"-i"
	run_distcp 	"320" "hdfs://$NN0.blue.ygrid"       $FED_WC_INDIR_SLASH   "hdfs://$NN1.blue.ygrid/$FED_WC_OUTDIR_SLASH"    "FQ hdfs default NN src to FQ default NN dest"		"-i"
	run_distcp 	"360" "hftp://$NN0.blue.ygrid:50070" $FED_WC_INDIR_SLASH   "hdfs://$NN2.blue.ygrid/$FED_WC_OUTDIR_SLASH"    "FQ hftp NN1 src to FQ NN2 dest"		"-i"


	hdftDisplayTestGroupMsg "Test distcp across multiple NNs, with different URI. Src from NN1/NN2. Output to NN0"
	run_distcp 	"400" "hdfs://$NN1.blue.ygrid"       $FED_WC_INDIR_SLASH    $FED_WC_OUTDIR_REL	                            "Default NN-home src to default NN-home dest"		"-update"
	run_distcp 	"410" "hdfs://$NN1.blue.ygrid"       $FED_WC_INDIR_SLASH   "hdfs://$NN0.blue.ygrid/$FED_WC_OUTDIR_SLASH"    "FQ hdfs default NN src to FQ default NN dest"		"-update"
	run_distcp 	"420" "hftp://$NN1.blue.ygrid:50070" $FED_WC_INDIR_SLASH   "hdfs://$NN0.blue.ygrid/$FED_WC_OUTDIR_SLASH"    "FQ hftp NN1 src to FQ NN2 dest"		"-update"
	run_distcp 	"430" "hdfs://$NN2.blue.ygrid"       $FED_WC_INDIR_SLASH    $FED_WC_OUTDIR_SLASH	                        "Default NN src to default NN dest"		"-update"
	run_distcp 	"450" "hftp://$NN2.blue.ygrid:50070" $FED_WC_INDIR_SLASH   "hdfs://$NN0.blue.ygrid/$FED_WC_OUTDIR_SLASH"    "FQ hftp NN1 src to FQ NN2 dest"		"-update"

	hdftDisplayTestGroupMsg "Test distcp across multiple NNs, with different URI. Src from NN1/NN2. Output to NN1/NN2"
	run_distcp 	"500" "hdfs://$NN1.blue.ygrid"       $FED_WC_INDIR_SLASH   "hdfs://$NN2.blue.ygrid/$FED_WC_OUTDIR_SLASH"    "Default NN-home src to default NN-home dest"		"-overwrite"
	run_distcp 	"510" "hdfs://$NN1.blue.ygrid"       $FED_WC_INDIR_SLASH   "hdfs://$NN2.blue.ygrid/$FED_WC_OUTDIR_SLASH"    "FQ hdfs default NN src to FQ default NN dest"		"-overwrite"
	run_distcp 	"520" "hftp://$NN1.blue.ygrid:50070" $FED_WC_INDIR_SLASH   "hdfs://$NN2.blue.ygrid/$FED_WC_OUTDIR_SLASH"    "FQ hftp NN1 src to FQ NN2 dest"		"-overwrite"
	run_distcp 	"530" "hdfs://$NN2.blue.ygrid"       $FED_WC_INDIR_SLASH   "hdfs://$NN1.blue.ygrid/$FED_WC_OUTDIR_SLASH"    "Default NN src to default NN dest"		"-overwrite"
	run_distcp 	"550" "hftp://$NN2.blue.ygrid:50070" $FED_WC_INDIR_SLASH   "hdfs://$NN1.blue.ygrid/$FED_WC_OUTDIR_SLASH"    "FQ hftp NN1 src to FQ NN2 dest"		"-overwrite"
