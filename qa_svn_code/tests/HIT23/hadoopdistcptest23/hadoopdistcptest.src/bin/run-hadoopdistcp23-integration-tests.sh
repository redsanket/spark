#!/bin/bash
# This is the Hadoop DistCp Test script for 0.23.3/2.0 release lines, it's based off 
...
function err () {
  echo "$* ..."
  echo "Hadoop DistCp23 integration test quitting ..."
  exit $HIT_FAIL
}
...
function copyData () {
   ...
   $HADOOP fs -mkdir ${HIT_HDFS_STORAGE_DIR}/validation ;
...
export PACKAGE_NAME='hadoopdistcptest23'
...
TESTS_LIST="runDistcpWithHdfsByName runDistcpWithHdfsByPath runDistcpWithHdfsDefaultByName runDistcpWithHftpByName runDistcpCheckVersion";
...
# Let's rumba
echo "Running hadoop DistCp23 integration tests..."
...
echo
echo "Finishing hadoop DistCp23 integration tests..."
