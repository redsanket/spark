...
echo "DISTCP TESTCASE1: Basic local Distcp using explicit FS type HDFS and a specific file by name"
...
$HADOOP distcp hdfs://$HIT_NN/tmp/in/file.txt hdfs://$HIT_NN/tmp/out 

# check the output
reference=`openssl dgst /tmp/file.txt | cut -d ' ' -f2`
target=`$HADOOP fs -cat  hdfs://$HIT_NN/tmp/out/file.txt | openssl dgst`
