...
COMPRESSED_DATA_BYTES=214748364
UNCOMPRESSED_DATA_BYTES=53687091
...
if [ ! -z ${USE_REAL_DATASET} ] ; then
  ...
  COMPRESSED_DATA_BYTES=214748364800
  # 500GB
  UNCOMPRESSED_DATA_BYTES=53687091200
fi
...
echo "GRIDMIXV2 DATA GENERATOR 1: ${HADOOP_HOME}/bin/hadoop jar \
  ...
  -D test.randomtextwrite.min_words_value=10 \
  -D test.randomtextwrite.max_words_value=1000 \
...
echo "GRIDMIXV2 DATA GENERATOR 2: ${HADOOP_HOME}/bin/hadoop jar \
  ${EXAMPLE_JAR} randomtextwriter \
  -D test.randomtextwrite.total_bytes=${COMPRESSED_DATA_BYTES} \
  -D test.randomtextwrite.bytes_per_map=$((${COMPRESSED_DATA_BYTES} / ${NUM_MAPS})) \
  -D test.randomtextwrite.min_words_key=5 \
  ...
  -D test.randomtextwrite.min_words_value=10 \
  -D test.randomtextwrite.max_words_value=10 \
  -D mapred.output.compress=true \
  -D mapred.map.output.compression.type=BLOCK \
  -outFormat org.apache.hadoop.mapred.SequenceFileOutputFormat \
  ${FIXCOMPSEQ}"
...
echo "GRIDMIXV2 DATA GENERATOR 3: ${HADOOP_HOME}/bin/hadoop jar \
  ${EXAMPLE_JAR} randomtextwriter \
