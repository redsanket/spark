#!/bin/bash
# This is the Hadoop DistCp Test script for 2.x release lines, it's based off 
# of the hadoopcoretest scripts, these tests reuse the data from the 20 design,
...
function err () {
  echo "$* ..."
  echo "Hadoop DistCp2dotX integration test quitting ..."
  exit $HIT_FAIL
}
...
function copyData () {
   KrbInit 
   $HADOOP fs -rmr ${HIT_HDFS_STORAGE_DIR}/validation ;
   $HADOOP fs -mkdir -p ${HIT_HDFS_STORAGE_DIR}/validation ;
   $HADOOP fs -copyFromLocal $DATA/data ${HIT_HDFS_STORAGE_DIR}/validation ;
   $HADOOP fs -copyFromLocal $TEST/bin ${HIT_HDFS_STORAGE_DIR}/validation ;
...
export PACKAGE_NAME='hadoopdistcptest2dotX'
...
export ROOT=$(getValue `/usr/local/bin/yinst env ${PACKAGE_NAME} | grep ^ROOT`)
...
export SUITE="Distcp_Integration_Test_Results"
# Getting the name of the configuration package and extracting type of the
# deploy (dev or prod)
hadoopConfigPackageName=`/usr/local/bin/yinst ls | grep HadoopConfig | sed -e 's/-.*$//'`
if [ "x$hadoopConfigPackageName" != "x" ]; then
  export DEPLOYMENT_TYPE=$(getValue `/usr/local/bin/yinst env ${hadoopConfigPackageName} | grep ^TODO_KERBEROS_ZONE`)
else
...
if [ "x$HIT_EXEC_ID" = "x" ]; then
  export HIT_EXEC_ID=hadoopdistcp-`date +%s`
else
  export HIT_EXEC_ID=hadoopdistcp-${HIT_EXEC_ID}
fi
if [ "x$HIT_HDFS_STORAGE_DIR" = "x" ]; then
  export HIT_HDFS_STORAGE_DIR=/tmp/${HIT_EXEC_ID}
fi
...
export HIT_TIMEOUT=600 #10 mins
 
# Actual set of test scripts
TESTS_LIST="runDistcpWithHdfsByName runDistcpWithHdfsByPath runDistcpWithHdfsDefaultByName runDistcpWithWebhdfsByName runDistcpCheckVersion";
...
# Let's rumba
echo "Running hadoop DistCp2dotX integration tests..."
...
echo
echo "Finishing hadoop DistCp2dotX integration tests..."
